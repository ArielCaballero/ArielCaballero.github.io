...
switch ($_GET["op"]){
	...
	case 'confirmar':
		$rspta=$agendarcita->confirmar($idcita, $_SESSION['idusuario']);
 		echo $rspta ? "Se confirmo la cita" : "La cita no se puede confirmar";
 		break;
	break;

	case 'cancelar':
		$rspta=$agendarcita->cancelar($idcita, $_SESSION['idusuario']);
 		echo $rspta ? "Se cancelo la cita" : "La cita no se puede cancelar";
 		break;
	break;
		...
		$rspta=$agendarcita->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
				...
				"6"=> ($reg->Condicion=='Pendiente')?'<button class="btn btn-danger" onclick="cancelar('.$reg->ID_Cita.','.$_SESSION['idusuario'].' )"><i class="fa fa-close"></i></button>'.
				'<button class="btn btn-success" onclick="confirmar('.$reg->ID_Cita.','.$_SESSION['idusuario'].' )"><i class="fa fa-check"></i></button>'.
				'<button class="btn btn-warning" onclick="mostrar('.$reg->ID_Cita.')"><i class="fa fa-pencil"></i></button>'
				:'<button class="btn btn-danger" onclick="cancelar('.$reg->ID_Cita.','.$_SESSION['idusuario'].' )" disabled><i class="fa fa-close"></i></button>'.
				'<button class="btn btn-success" onclick="confirmar('.$reg->ID_Cita.','.$_SESSION['idusuario'].' )" disabled><i class="fa fa-check"></i></button>'.
				'<button class="btn btn-warning" onclick="mostrar('.$reg->ID_Cita.')" disabled><i class="fa fa-pencil"></i></button>'
 				);
 		}
 		$results = array(
